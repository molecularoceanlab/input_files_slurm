...
#   DESCRIPTION
#   write plumed.dat to start eq2 stretch mtd infinite from starting position on cluster 1 last frame on vmd clustring
...
# generate plumed.dat into crys directory and edit amor.dat from it
cd crys
...
protid=`awk '{if (!($1 ~ /UNK/ || $1 ~ /CL/ || $1 ~ /NA/ || $1 ~ /SOL/)) print $3}' $gro | tail -2 | head -1`
...
UPPER_WALLS ARG=d0 AT=0.3 KAPPA=5000 EXP=2 EPS=1 OFFSET=0 LABEL=d0_uwall        # 4A min distance from ser
UPPER_WALLS ARG=h0 AT=0.3 KAPPA=5000 EXP=2 EPS=1 OFFSET=0 LABEL=h0_uwall        # 4A min distance from hole
UPPER_WALLS ARG=h1 AT=0.3 KAPPA=5000 EXP=2 EPS=1 OFFSET=0 LABEL=h1_uwall        # 4A min distance from hole
UPPER_WALLS ARG=s0 AT=0.3 KAPPA=5000 EXP=2 EPS=1 OFFSET=0 LABEL=s0_uwall        # 4A min distance from hole
...
sed \
    -e "s/LOWER_WALLS ARG=d1 AT=4.0 KAPPA=2550 EXP=2 EPS=1 OFFSET=0 LABEL=d1_lwall/LOWER_WALLS ARG=d1 AT=2.4 KAPPA=2550 EXP=2 EPS=1 OFFSET=0 LABEL=d1_lwall/" \
    -e "s/crystalline/amorphous/" \
    eq2_crys.dat > ../amor/eq2_amor.dat
