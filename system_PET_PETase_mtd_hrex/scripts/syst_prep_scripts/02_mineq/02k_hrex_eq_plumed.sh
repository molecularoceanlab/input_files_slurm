...
# DESCRIPTION
# write plumed file for equilibrium in hrex after pulling
# from main directory
# syst=7nei
# cd systs/$syst/02_mineq; echo $syst; ../../../scripts/02_mineq/02k_hrex_eq_plumed.sh $syst; cd ../../..
# for syst in $(ls systs); do if [ "$syst" != "6eqe" ]; then cd systs/$syst/02_mineq; echo $syst; ../../../scripts/02_mineq/02k_hrex_eq_plumed.sh $syst; cd ../../..; fi; done
...
PYTHON_SCRIPT_PATH="../../../templates/prep/references_petase.py"
...
# get aa references references for plumed file
...
# get atom indexes for plumed file
...
[ -d "hrex" ] && echo -e "Directory hrex exists." || (echo -e "creating directory hrex" && mkdir hrex)
...
awk -v NAME=UNK '/ atoms /,/ bonds /{if (NF >= 8 && $4==NAME ) $2 = $2 "_" }1' pull.top > hrex/lighot.top
# only heavy atoms ???
...
LOWER_WALLS ARG=d1 AT=23 KAPPA=250 EXP=2 EPS=1 OFFSET=0 LABEL=lwall     # prevents lig to wrap
UPPER_WALLS ARG=d0 AT=5  KAPPA=500 EXP=2 EPS=1 OFFSET=0 LABEL=uwall     # prevents lig to leave AS

RESTRAINT ...
ARG=h0,h1,drms0
AT=3,3,0
KAPPA=5000,5000,5000
LABEL=rstr
...
