...
# === PARAMETERS ===
base_path = "hrex_mtd/r0/torsions"
ligs = ['amor','crys']
...
print(f"{trans_range=}")
print(f"syst\tdist_lw\tdist_av\t#tot\t#count\t#trans\t#gauche\t   ratio(t:g)\tbias_av\tforc_lw\tforc_av")
for lig in ligs:
    colvar = ref.load_plumed(f'{lig}/{base_path}/torsions_cv')
    tot = len(colvar["time"]) #
    dw_av = round(np.mean(colvar["d1"]),1)
    bw_av = round(np.mean(colvar["d1_lwall.bias"]),1)
    fw_av = round(np.mean(colvar["d1_lwall.force2"]),1)
    ...
    print(f"{lig}\t{at_val}\t{dw_av}\t{tot_ts}\t{counts}\t{trans_count}\t{gauche_count}\t{ratio_t}\t{ratio_g}\t{bw_av}\t{kappa_val}\t{fw_av}")
